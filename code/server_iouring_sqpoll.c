// =============================================================================
//  server_iouring_sqpoll.c  —  TCP Echo Server using io_uring with SQPOLL
// =============================================================================
//
...
static int ring_init(io_uring_t *r, unsigned depth) {
    ...
    r->sq_flags     = (unsigned *)((char *)sq_ring + params.sq_off.flags);
...
static int ring_submit(io_uring_t *r, unsigned wait_nr) {
    unsigned flags = 0;
    unsigned sq_flags = atomic_load_explicit((_Atomic unsigned *)r->sq_flags, memory_order_acquire);
    ...
    if (flags == 0) {
        return 0; // SQPOLL thread is awake and no wait required; zero syscalls!
    }
    
    unsigned tail = atomic_load_explicit((_Atomic unsigned *)r->sq_tail, memory_order_relaxed);
    unsigned head = atomic_load_explicit((_Atomic unsigned *)r->sq_head, memory_order_relaxed);
